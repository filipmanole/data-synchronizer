...
const get: APIGatewayProxyHandler = async (event): Promise<APIGatewayProxyResult> => {
  try {
    ...
    if(!result.Item) return { statusCode: 200, body: JSON.stringify({ msg: 'No existing file' })};
